...
bool retrievingProductsForSelectedBrand = false;
  @override
  void initState() {
...
//    productBrands.add('HEERA');
    ...
    brandSearchResults.sort((dynamic a, dynamic b) {
...
void retrieveProductsForBrand(){
    setState(() {
      retrievingProductsForSelectedBrand = true;
    });

    barCodeSearchResults = List<ProductBasicDetails>();
    barCodeSearchResultsMap = Map<int, ProductBasicDetails>();

    FirebaseDatabase.instance.reference().child('stores/KIRANAWALA_STORE_11/products').orderByChild('brand').equalTo(selectedBrand).once().then((snapshot){
      print(snapshot);
      ...
      setState(() {
        retrievingProductsForSelectedBrand = false;
      });
  ...
  Widget build(BuildContext context) {
    if(retrievingProductsForSelectedBrand)
      return
        Scaffold(
          appBar:
          AppBar(
